...
Reject crawler, indexer, or dialect-only open memory marketplaces whose submission
path is only a hosted claim form, upload UI, or account-gated directory,
token-compressed skill file, team-memory sync artifact, or hosted memory config
unless the repository exposes a seed, source, catalog, or registry file that can be changed by PR
and can point to a source-linked Hermes Tweet package entry.
