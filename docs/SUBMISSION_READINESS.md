...
Reject generic skill-index import registries, GitHub-topic-only discovery CLIs, VS Code Copilot marketplace publisher extensions, external-PR-closing plugin marketplaces, README-only placeholder registries, zip-only skill wrapper repos, source-owned generated catalogs, domain-specific skill bundles, owner-specific plugin bundles, tenant or fleet skill registries, generic skill-registry submission guides, onboarding docs, and wishlist issues when they describe review policy, local plugin setup, learning paths, import APIs, publication tooling, external submission forms, member-only contribution lanes, or topic-based skill discovery but do not expose a PR-editable registry source file, catalog seed, or source-list lane for third-party Hermes Tweet package entries.
