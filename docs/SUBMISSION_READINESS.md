...
Reject generic skill-index import registries, owner-specific plugin bundles, generic skill-registry submission guides, onboarding docs, and wishlist issues when they describe review policy, local plugin setup, learning paths, or import APIs but do not expose a PR-editable registry source file, catalog seed, or source-list lane for third-party Hermes Tweet package entries.
