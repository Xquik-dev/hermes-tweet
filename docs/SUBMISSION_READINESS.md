...
Reject crawler, indexer, dialect-only open memory marketplaces, site-content production skill catalogs, admin-synced in-app skill libraries, and environment-configured skill ingestion services whose submission path is only a hosted claim form, upload UI, or account-gated directory, token-compressed skill file, team-memory sync artifact, hosted memory config, product content workflow, app-private skill body, or locally configured ingest target unless the repository exposes a seed, source, catalog, or registry file that can be changed by PR and can point to a source-linked Hermes Tweet package entry.
