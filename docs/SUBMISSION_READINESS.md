...
Reject generic skill-index import registries, GitHub-topic-only discovery CLIs, domain-specific skill bundles, owner-specific plugin bundles, tenant or fleet skill registries, generic skill-registry submission guides, onboarding docs, and wishlist issues when they describe review policy, local plugin setup, learning paths, import APIs, or topic-based skill discovery but do not expose a PR-editable registry source file, catalog seed, or source-list lane for third-party Hermes Tweet package entries.
...
Reject tool-surface dump catalogs, agent tool indexes, agent capability registries, tool-vault directories, tools YAML registries, catalog drift analyzers, database-backed MCP skill registry servers, and internal tool-registry backends such as agent app tool dumps, toolmirror-style diff tools, or organizational LLM tool registry apps when they document runtime-owned tool schemas, agent protocol manifests, runtime/API code without a PR-editable source catalog lane, or capability schemas instead of accepting source-linked third-party Hermes Tweet package entries.
